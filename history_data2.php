...
$days = $Year.$Month.$Day;
$p1 = $dbh->prepare("select * from `history_data` where days=:days and stock_type=2 and warrant_type=1 limit 1");
$p2 = $dbh->prepare("insert into `history_data` (`days`,`stock_id`,`stock_name`,`deal_amount`,`start_price`,`highest_price`,
		...
		:deal_amount,:start_price,:highest_price,:lowest_price,:end_price,2,1,now())");
